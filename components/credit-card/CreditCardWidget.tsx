...
export default function CreditCardWidget({ card, currentInvoice, onEdit, onDelete, onViewInvoice }: CreditCardWidgetProps) {
    ...
    const [imgError, setImgError] = useState(false); const availableLimit = card.limit - currentInvoice;
        ...
        >
                ...
                <div className="relative">
                    ...
                    {showMenu && (
                        <>
                            ...
                            />
                            <div className="absolute right-0 top-6 z-20 bg-white rounded-lg shadow-lg py-1 w-32 overflow-hidden">                                <button
                                onClick={() => {
                                    onViewInvoice?.();
                                    setShowMenu(false);
                                }}
            ...
            <div className={`mt-4 pt-3 border-t border-white/10 flex justify-between text-xs ${subTextColor}`}>
                ...
                <div className="text-right">
                    <p className="text-[10px] opacity-80">{t('due_day')}</p>                    <p className="font-medium">Dia {card.dueDay}</p>
                </div>
            </div>
        </div>
    );
